...
const LOGROS_DEF = [
  // ── Partidos ──
  { id: 'first_win',   emoji: '🏆', nombre: 'Primera victoria',      desc: 'Gana tu primer partido de Arena',           tipo: 'arena_win',     objetivo: 1,     reward: { coins: 200  } },
  { id: 'win_10',      emoji: '⚔️', nombre: 'Guerrero',               desc: 'Gana 10 partidos de Arena',                tipo: 'arena_win',     objetivo: 10,    reward: { coins: 500  } },
  { id: 'win_50',      emoji: '🔥', nombre: 'Leyenda de Arena',       desc: 'Gana 50 partidos de Arena',                tipo: 'arena_win',     objetivo: 50,    reward: { coins: 2000 } },
  { id: 'friendly_10', emoji: '🤝', nombre: 'Jugador social',         desc: 'Juega 10 amistosos',                       tipo: 'friendly_play', objetivo: 10,    reward: { coins: 300  } },
  // ── Constancia ──
  { id: 'streak_3',   emoji: '🔥', nombre: 'En racha',               desc: 'Alcanza 3 días seguidos en !daily',         tipo: 'daily_streak',  objetivo: 3,     reward: { coins: 150  } },
  { id: 'streak_7',   emoji: '⚡', nombre: 'Semana perfecta',        desc: 'Alcanza 7 días seguidos en !daily',         tipo: 'daily_streak',  objetivo: 7,     reward: { coins: 400  } },
  { id: 'streak_30',  emoji: '👑', nombre: 'Mes de fuego',           desc: 'Alcanza 30 días seguidos en !daily',        tipo: 'daily_streak',  objetivo: 30,    reward: { coins: 2000 } },
  // ── Colección ──
  { id: 'first_pack', emoji: '📦', nombre: 'Primer sobre',           desc: 'Abre tu primer pack',                       tipo: 'packs_opened',  objetivo: 1,     reward: { coins: 100  } },
  { id: 'packs_10',   emoji: '🎁', nombre: 'Coleccionista',          desc: 'Abre 10 packs',                             tipo: 'packs_opened',  objetivo: 10,    reward: { coins: 400  } },
  { id: 'packs_50',   emoji: '🌟', nombre: 'Fanático de los sobres', desc: 'Abre 50 packs',                             tipo: 'packs_opened',  objetivo: 50,    reward: { coins: 1500 } },
  { id: 'legend_card',emoji: '💎', nombre: 'Legendario',             desc: 'Consigue tu primera carta Legendaria',      tipo: 'rarity_owned',  objetivo: 'Legendario', reward: { coins: 800  } },
  { id: 'icon_card',  emoji: '⭐', nombre: 'El elegido',             desc: 'Consigue tu primera carta Icon',            tipo: 'rarity_owned',  objetivo: 'Icon',       reward: { coins: 3000 } },
  { id: 'full_club',  emoji: '🏟️', nombre: 'Club completo',          desc: 'Llena tu club con 20 jugadores',           tipo: 'club_full',     objetivo: 1,     reward: { coins: 600  } },
  // ── Economía ──
  { id: 'rich_1k',    emoji: '💰', nombre: 'Primeros ahorros',       desc: 'Acumula 1,000 monedas',                     tipo: 'coins_total',   objetivo: 1000,  reward: { coins: 0    } },
  { id: 'rich_10k',   emoji: '💵', nombre: 'Millonario junior',      desc: 'Acumula 10,000 monedas',                    tipo: 'coins_total',   objetivo: 10000, reward: { coins: 500  } },
  { id: 'rich_50k',   emoji: '🤑', nombre: 'Magnate',                desc: 'Acumula 50,000 monedas',                    tipo: 'coins_total',   objetivo: 50000, reward: { coins: 2500 } },
  { id: 'sell_10',    emoji: '🏪', nombre: 'Vendedor',               desc: 'Vende 10 cartas en el market',              tipo: 'cards_sold',    objetivo: 10,    reward: { coins: 350  } },
  // ── ELO ──
  { id: 'elo_1200',   emoji: '🔷', nombre: 'Platino',                desc: 'Alcanza 1,200 ELO',                         tipo: 'elo_reached',   objetivo: 1200,  reward: { coins: 600  } },
  { id: 'elo_1500',   emoji: '💎', nombre: 'Diamante',               desc: 'Alcanza 1,500 ELO',                         tipo: 'elo_reached',   objetivo: 1500,  reward: { coins: 1500 } },
  { id: 'elo_1800',   emoji: '👑', nombre: 'Campeón supremo',        desc: 'Alcanza 1,800 ELO',                         tipo: 'elo_reached',   objetivo: 1800,  reward: { coins: 4000 } },
  // ── Duelos ──
  { id: 'first_duel', emoji: '🥊', nombre: 'Primer duelo',           desc: 'Participa en tu primer duelo con apuesta',  tipo: 'duels_played',  objetivo: 1,     reward: { coins: 100  } },
  { id: 'duel_win_5', emoji: '💸', nombre: 'Apostador nato',         desc: 'Gana 5 duelos con apuesta',                 tipo: 'duels_won',     objetivo: 5,     reward: { coins: 800  } },
];
...
function checkLogros(uid, eventType, value = 1) {
  ...
  if (!u.logrosStats) u.logrosStats = {};

  const newlyUnlocked = [];

  for (const logro of LOGROS_DEF) {
    if (u.logros[logro.id]?.unlocked) continue;
    if (logro.tipo !== eventType) continue;
    if (!u.logrosStats[logro.id]) u.logrosStats[logro.id] = 0;
    ...
    if (logro.tipo === 'rarity_owned') {
      ...
      u.logrosStats[logro.id] += value;
      meetsCondition = u.logrosStats[logro.id] >= logro.objetivo;
    }

    if (meetsCondition) {
      u.logros[logro.id] = { unlocked: true, date: Date.now() };
    ...
    await message.channel.send({
      embeds: [{
        ...
        description: [
          ...
          ``,
          l.reward?.coins > 0
            ? `💰 **+${l.reward.coins.toLocaleString()} 💰** añadidos a tu balance.`
            : `✅ Logro registrado.`,
        ].join('\n'),
        footer: { text: 'Usa !logros para ver todos tus logros' },
...
// ── FAZE CHAMPIONS ──
mkp("Barco FZ",     85, "ST", "FazeChampions", {PAC:90,SHO:91,PAS:82,DRI:88,DEF:52,PHY:86}),
mkp("Paul FZ",      88, "AM", "FazeChampions", {PAC:87,SHO:88,PAS:90,DRI:92,DEF:60,PHY:85}),
mkp("Dqvid FZ",     87, "DM", "FazeChampions", {PAC:84,SHO:78,PAS:89,DRI:86,DEF:91,PHY:88}),
mkp("Short FZ",     86, "GK", "FazeChampions", {DIV:85,REF:87,HAN:86,KIC:85,POS:89}),
mkp("Shepard FZ",88, "ST", "FazeChampions", {PAC:88,SHO:93,PAS:80,DRI:86,DEF:52,PHY:89}),
mkp("Zombot FZ", 87, "GK", "FazeChampions", {DIV:88,REF:90,HAN:87,KIC:86,POS:89}),
mkp("Fallen FZ", 87, "DM", "FazeChampions", {PAC:84,SHO:78,PAS:87,DRI:85,DEF:90,PHY:87}),
mkp("Saskee FZ", 87, "AM", "FazeChampions", {PAC:88,SHO:89,PAS:82,DRI:90,DEF:52,PHY:83}),
mkp("Atlanta",   84, "AM", "FazeChampions", {PAC:85,SHO:86,PAS:89,DRI:90,DEF:58,PHY:82}),
mkp("Apolo FZ",  86, "DM", "FazeChampions", {PAC:84,SHO:85,PAS:88,DRI:89,DEF:54,PHY:80}),
mkp("Zae",       87, "ST", "FazeChampions", {PAC:89,SHO:88,PAS:80,DRI:87,DEF:48,PHY:84}),
mkp("Nunf FZ",   85, "AM", "FazeChampions", {PAC:83,SHO:84,PAS:88,DRI:90,DEF:52,PHY:80}),
...
mkp("Doxing",     95, "AM", "Icon", {PAC:93,SHO:91,PAS:97,DRI:98,DEF:65,PHY:89}),
  ...
  mkp("Sekai",        90, "DM", "Legendario", {PAC:94,SHO:94,PAS:93,DRI:97,DEF:98,PHY:97}),
  mkp("Czerro",       91, "DM", "Legendario", {PAC:97,SHO:91,PAS:95,DRI:97,DEF:98,PHY:95}),
  ...
  mkp("Levaldo",      89, "DM", "Legendario", {PAC:86,SHO:87,PAS:89,DRI:90,DEF:95,PHY:94}),
  mkp("Mirkoller",    90, "GK", "Legendario", {DIV:93,REF:98,HAN:94,KIC:94,POS:96}),
  mkp("Pixel",        91, "ST", "Legendario", {PAC:94,SHO:96,PAS:93,DRI:90,DEF:80,PHY:94}),
  mkp("Quesonub",     89, "DM", "Legendario", {PAC:90,SHO:89,PAS:94,DRI:90,DEF:96,PHY:93}),
  mkp("Aoi",          89, "DM", "Legendario", {PAC:90,SHO:88,PAS:91,DRI:94,DEF:94,PHY:94}),
  mkp("Father",       89, "GK", "Legendario", {DIV:90,REF:93,HAN:90,KIC:90,POS:91}),
  mkp("Kayn",         91, "AM", "Legendario", {PAC:90,SHO:93,PAS:93,DRI:95,DEF:80,PHY:91}),
  mkp("Lyreco",       91, "ST", "Legendario", {PAC:93,SHO:95,PAS:87,DRI:90,DEF:76,PHY:91}),
  mkp("Paul",         90, "AM", "Legendario", {PAC:92,SHO:92,PAS:93,DRI:95,DEF:77,PHY:92}),
  mkp("Dqvid",        86, "DM", "Legendario", {PAC:84,SHO:80,PAS:90,DRI:87,DEF:93,PHY:90}),
  ...
  mkp("Kaiser",       88, "ST", "Legendario", {PAC:92,SHO:95,PAS:84,DRI:93,DEF:70,PHY:90}),
  mkp("Pechuga",      87, "AM", "Legendario", {PAC:90,SHO:87,PAS:92,DRI:89,DEF:80,PHY:92}),
  mkp("Shepard",      88, "ST", "Legendario", {PAC:91,SHO:97,PAS:85,DRI:87,DEF:70,PHY:92}),
  mkp("Zombot",       87, "GK", "Legendario", {DIV:88,REF:88,HAN:86,KIC:87,POS:87}),

  mkp("Bachira",      84, "AM", "Epico",      {PAC:83,SHO:85,PAS:89,DRI:96,DEF:60,PHY:85}),
  mkp("Fallen",       85, "DM", "Epico",      {PAC:82,SHO:80,PAS:85,DRI:86,DEF:87,PHY:86}),
  mkp("Roki",         85, "AM", "Epico",      {PAC:86,SHO:87,PAS:85,DRI:95,DEF:70,PHY:90}),
  mkp("Zae",          85, "AM", "Epico",      {PAC:83,SHO:85,PAS:89,DRI:87,DEF:67,PHY:84}),
  mkp("N+23",         85, "GK", "Epico",      {DIV:86,REF:89,HAN:85,KIC:81,POS:87}),
  mkp("Pain",         84, "DM", "Epico",      {PAC:81,SHO:70,PAS:82,DRI:88,DEF:90,PHY:86}),
  mkp("Pinotek",      84, "AM", "Epico",      {PAC:82,SHO:84,PAS:88,DRI:89,DEF:56,PHY:71}),
  mkp("Sixer",        84, "DM", "Epico",      {PAC:81,SHO:68,PAS:82,DRI:80,DEF:89,PHY:86}),
  mkp("Smurf",        84, "GK", "Epico",      {DIV:85,REF:88,HAN:84,KIC:80,POS:86}),
  mkp("Cosmik",       84, "ST", "Epico",      {PAC:80,SHO:94,PAS:75,DRI:84,DEF:64,PHY:87}),
  ...
  mkp("Allan Saint",  82, "DM", "Epico",      {PAC:79,SHO:66,PAS:80,DRI:78,DEF:87,PHY:84}),
  mkp("Korai",        82, "DM", "Epico",      {PAC:79,SHO:66,PAS:80,DRI:78,DEF:87,PHY:84}),
  mkp("Lawliet",      83, "AM", "Epico",      {PAC:80,SHO:80,PAS:83,DRI:83,DEF:53,PHY:74}),
  mkp("Metzi",        85, "DM", "Epico",      {PAC:80,SHO:82,PAS:86,DRI:87,DEF:56,PHY:70}),
  mkp("Nocke",        82, "AM", "Raro",      {PAC:80,SHO:82,PAS:86,DRI:87,DEF:56,PHY:75}),
  mkp("Saskee",       82, "ST", "Raro",      {PAC:80,SHO:82,PAS:86,DRI:89,DEF:65,PHY:70}),
  mkp("369",          82, "ST", "Raro",      {PAC:86,SHO:86,PAS:74,DRI:82,DEF:46,PHY:78}),
  mkp("Rose",         81, "ST", "Raro",      {PAC:86,SHO:88,PAS:74,DRI:79,DEF:46,PHY:87}),
  mkp("Anon",         81, "DM", "Raro",      {PAC:78,SHO:65,PAS:79,DRI:77,DEF:86,PHY:83}),
  mkp("Paloma",       80, "DM", "Raro",      {PAC:77,SHO:64,PAS:78,DRI:76,DEF:85,PHY:82}),
  ...
  mkp("Barita",       77, "GK", "Comun",      {DIV:78,REF:81,HAN:77,KIC:74,POS:79}),
  mkp("Dan",          77, "GK", "Comun",      {DIV:78,REF:81,HAN:77,KIC:74,POS:79}),
  mkp("Kantz",        77, "AM", "Comun",      {PAC:78,SHO:77,PAS:81,DRI:82,DEF:51,PHY:67}),
  mkp("Lxthomas",     77, "AM", "Comun",      {PAC:75,SHO:77,PAS:81,DRI:82,DEF:51,PHY:67}),
  mkp("Nunf",         77, "AM", "Comun",      {PAC:75,SHO:77,PAS:81,DRI:89,DEF:51,PHY:67}),
  mkp("Silva",        77, "GK", "Comun",      {DIV:78,REF:81,HAN:77,KIC:74,POS:79}),
  mkp("Slurpy",       81, "GK", "Raro",      {DIV:78,REF:81,HAN:77,KIC:74,POS:79}),
  mkp("Moonsky",      81, "ST", "Raro",      {PAC:83,SHO:82,PAS:70,DRI:78,DEF:44,PHY:74}),
  mkp("Frist",        77, "ST", "Comun",      {PAC:81,SHO:83,PAS:68,DRI:73,DEF:42,PHY:72}),
  mkp("Insane",       77, "ST", "Comun",      {PAC:81,SHO:80,PAS:68,DRI:76,DEF:42,PHY:72}),
  mkp("Nova",         77, "ST", "Comun",      {PAC:81,SHO:80,PAS:68,DRI:76,DEF:42,PHY:72}),
  mkp("Torrez",       77, "ST", "Comun",      {PAC:81,SHO:80,PAS:68,DRI:76,DEF:42,PHY:72}),
  mkp("Aj",           77, "ST", "Comun",      {PAC:80,SHO:79,PAS:67,DRI:75,DEF:42,PHY:71}),
  mkp("Barco",        77, "ST", "Comun",      {PAC:70,SHO:75,PAS:70,DRI:75,DEF:42,PHY:71}),
  ...
  mkp("Iancillo",     76, "GK", "Comun",      {DIV:76,REF:79,HAN:75,KIC:72,POS:77}),
  ...
  mkp("Wheezy",       76, "GK", "Comun",      {DIV:76,REF:79,HAN:75,KIC:72,POS:77}),
  ...
  mkp("Chino Huerta", 74, "ST", "Comun",      {PAC:78,SHO:77,PAS:65,DRI:73,DEF:41,PHY:70}),
  mkp("Ian+",         74, "AM", "Comun",      {PAC:72,SHO:74,PAS:78,DRI:79,DEF:48,PHY:64}),
  mkp("Kripps",       74, "AM", "Comun",      {PAC:72,SHO:74,PAS:78,DRI:79,DEF:48,PHY:64}),
  mkp("L.Diaz",       74, "GK", "Comun",      {DIV:74,REF:77,HAN:73,KIC:70,POS:75}),
  mkp("LianMoon",     74, "AM", "Comun",      {PAC:71,SHO:73,PAS:77,DRI:78,DEF:48,PHY:63}),
  mkp("Ly.",          74, "AM", "Comun",      {PAC:71,SHO:73,PAS:77,DRI:78,DEF:48,PHY:63}),
  mkp("Montiel",      75, "ST", "Comun",      {PAC:78,SHO:77,PAS:65,DRI:73,DEF:41,PHY:70}),
  mkp("Patatahot",    74, "GK", "Comun",      {DIV:74,REF:77,HAN:73,KIC:70,POS:75}),
  mkp("Polmito",      74, "ST", "Comun",      {PAC:66,SHO:83,PAS:65,DRI:68,DEF:61,PHY:78}),
  mkp("Rambo",        74, "GK", "Comun",      {DIV:74,REF:77,HAN:73,KIC:70,POS:75}),
  ...
  mkp("Theviruz",     81, "ST", "Raro",      {PAC:78,SHO:77,PAS:65,DRI:73,DEF:41,PHY:70}),
...
const packs = {
 ...
 fazechampions: { price: 45000, label: 'FaZe Champions', emoji: '🔴', rarities: ['FazeChampions'] },
};

const SELL_PRICES = { "Comun": 230, "Raro": 1150, "Epico": 3650, "Legendario": 7250, "Icon": 40000, "FazeChampions": 22000 };
const MARKET_MIN_PRICE = { "Comun": 300, "Raro": 1900, "Epico": 5000, "Legendario": 17000, "Icon": 100000, "FazeChampions": 50000 };
...
const PACK_VISUAL = {
  ...
  fazechampions: { primary: '#8B0000', secondary: '#2d0000', accent: '#FF2222', glow: '#CC0000', shine: '#FF4444', dark: '#0d0000' },
};
...
if (rarity === "FazeChampions") return {
  cardTop:    '#1a0000', cardMid:    '#2d0000', cardBot:    '#0d0000',
  nameBar:    '#8B0000', statsArea:  '#1a0000', border:     '#CC0000',
  glow:       '#FF0000', ratingCol:  '#FF4444', posCol:     '#FF6666',
  nameCol:    '#FFFFFF', statNum:    '#FFFFFF', statLabel:  '#FF4444', shine: '#FF2222',
};
...
async function drawShowcaseCard(player) {
  ...
  const isWC = player.rarity === 'WorldCup' || player.rarity === 'Icon' || player.rarity === 'FazeChampions';
  ...
  if (isWC) {
    ...
    ctx.shadowBlur = 8;
    const badgeText = player.rarity === 'Icon' ? '✦  ICON  ✦' : player.rarity === 'FazeChampions' ? '✦  FAZE CHAMPIONS  ✦' : '✦  WORLD CUP CHAMPIONS  ✦';
...
// ── LOGO DEL CLUB / FAZE ──
  // Posición: debajo del texto de posición, lado izquierdo — igual que en !show
    const logoR  = 22;
  const logoCX = cx + 40;
  const logoCY = cy + 155 + topOffset;

  if (player.rarity === 'FazeChampions') {
    try {
      const fazeImg = await loadImage('/app/assets/faze-logo.png');
      ctx.save();
      ...
      ctx.clip();
      ctx.drawImage(fazeImg, logoCX - logoR, logoCY - logoR, logoR * 2, logoR * 2);
      ctx.restore();
      ctx.save();
      ctx.beginPath();
      ctx.arc(logoCX, logoCY, logoR, 0, Math.PI * 2);
      ...
      console.error('Error logo FaZe showcase:', e.message);
    }
  } else {
    const ownerEntry = Object.entries(data).find(([uid, udata]) =>
      (udata.players || []).some(p => p.name === player.name)
    );
    if (ownerEntry && ownerEntry[1].clubLogo) {
      const logoBuffer = Buffer.from(ownerEntry[1].clubLogo, 'base64');
      await drawClubLogo(ctx, logoBuffer, logoCX, logoCY, logoR);
    }
  }
...
// ── LOGO DEL CLUB / FAZE ──
  const logoSmallR  = 11;
  const logoSmallCX = ox + 20;
  const logoSmallCY = oy + 75;
  ...
  if (player.rarity === 'FazeChampions') {
    try {
      ...
      ctx.save();
      ctx.beginPath();
      ctx.arc(logoSmallCX, logoSmallCY, logoSmallR, 0, Math.PI * 2);
      ctx.clip();
      ctx.drawImage(fazeImg,
        logoSmallCX - logoSmallR,
        logoSmallCY - logoSmallR,
        logoSmallR * 2,
        logoSmallR * 2
      );
      ctx.restore();
      ...
      console.error('Error logo FaZe drawCard:', e.message);
    }
  } else {
    const ownerEntrySmall = Object.entries(data).find(([uid, udata]) =>
      (udata.players || []).some(p => p.name === player.name)
    );
    if (ownerEntrySmall && ownerEntrySmall[1].clubLogo) {
      const logoBufferSmall = Buffer.from(ownerEntrySmall[1].clubLogo, 'base64');
      await drawClubLogo(ctx, logoBufferSmall, logoSmallCX, logoSmallCY, logoSmallR);
    }
  }
...
async function generatePackShopCanvas() {
  ...
  {
  key: 'fazechampions', label: 'FAZE CHAMPS', sublabel: 'Jugadores FaZe Champions',
  price: '45000', sellVal: '22000', rarity: 'FAZE', cmd: '!buy faze',
  bg1: '#1a0000', bg2: '#0d0000', topGlow: '#CC0000', accent: '#8B0000',
  accentLight: '#FF4444', border1: '#5C0000', border2: '#CC0000',
  badgeBg: '#2d0000', badgeText: '#FF4444',
  priceColor: '#FF4444', particles: ['#FF4444', '#CC0000', '#8B0000'],
},
  ];

  const cardW = 175, cardH = 400;
  const startX = (W - (cardW * 6 + 30 * 5)) / 2;
  const cardY = 95;
...
const helpPages = [
  {
    ...
    fields: [
      ...
      { name: '🎒 `!mypacks`',     value: 'Ver cuántos packs tienes disponibles', inline: false },
      { name: '🎮 `!open <tipo>` / `!o <tipo>`', value: 'Abrir pack con animación en vivo paso a paso\n🥉 bronze · ⚪ silver · 🥇 gold · 💎 legend · 🏆 icon · 🔴 fazechampions', inline: false },
    ],
...
{ name: '🔄 `!trade @usuario <tu carta> por <su carta>`',
  ...
  },
  {
    ...
    fields: [
      ...
      { name: '⚔️ `!duelo @usuario <apuesta>`', value: 'Reta a alguien con monedas en juego\nEl ganador se lleva todo · Mín **100 💰** · Cooldown 5 min', inline: false },
   { name: '🏆 `!logros`', value: 'Ver tus logros y progreso · Recompensas automáticas al desbloquear', inline: false },
   { name: '📊 `!coleccion`', value: 'Ver qué % de cartas posees, desglose por rareza y tus mejores cartas', inline: false },
...
{ name: '🎯 `!quests` / `!misiones`',
  ...
  fields: [
    ...
    { name: '`!rul <cantidad> <opción>`',  value: 'Ruleta · Elige color o número\n🔴 Negro/Rojo **x2** · 🟢 Verde **x35** · 🎯 Número exacto **x35** · Cooldown **10 min**', inline: false },
...
if (isBanned(message.author.id)) {
 ...
 if (!data[userId]) {
    data[userId] = { coins: 1800, players: [], team: [], teamName: message.author.username + "'s FC", packs: { silver:0, bronze:0, gold:0, legend:0, icon:0, fazechampions:0 }, elo: 1000, daily: { lastClaim:0, streak:0 }, clubLogo: null };
    saveData();
  }
  ...
  if (u.packs.fazechampions === undefined) u.packs.fazechampions = 0;
  saveData();
...
if (elapsed < CLAIM_COOLDOWN_MS) {
  const remaining = CLAIM_COOLDOWN_MS - elapsed;
    ...
    if (packLines.length > 0) lines.push(``, `🎉 **¡Packs desbloqueados por racha!**`, ...packLines, ``, `📦 Revisa tu inventario con \`!mypacks\``);
    if (streakBar) lines.push(streakBar);
  ...
  if (cmd === '!packs') {
   ...
   new ButtonBuilder()
    .setCustomId(`shop_buy_fazechampions_${userId}`)
    .setLabel('🔴 FaZe Champs — 45000 💰')
...
`🔴 FaZe Champions: **${user.packs.fazechampions||0}**`
    );
  }
...
const validExtras = ['par', 'impar', 'docena1', 'docena2', 'docena3'];
...
if (!isColor && !isExtra && !isNumber) return message.reply('❌ Elige un color, par/impar, docena1/2/3 o número del 0 al 36.');
...
if (forcedResult !== null && (forcedForUser === userId || forcedForUser === 'global')) {
  ...
  if (choice === resultColor) {
    won = true;
    multiplier = choice === 'verde' ? 35 : 2;
  }
} else if (isExtra) {
  if (result === 0) {
    won = false; // el 0 pierde en par/impar/docenas
  } else if (choice === 'par' && result % 2 === 0) {
    won = true; multiplier = 2;
  } else if (choice === 'impar' && result % 2 !== 0) {
    won = true; multiplier = 2;
  } else if (choice === 'docena1' && result >= 1 && result <= 12) {
    won = true; multiplier = 3;
  } else if (choice === 'docena2' && result >= 13 && result <= 24) {
    won = true; multiplier = 3;
  } else if (choice === 'docena3' && result >= 25 && result <= 36) {
    won = true; multiplier = 3;
  }
} else {
  ...
  await spinMsg.edit({
    embeds: [{
      ...
      fields: [
        ...
        { name: '🟢 Verde', value: 'x35', inline: true },
        { name: '🎯 Número exacto', value: 'x35', inline: true },
        { name: '🔢 Par/Impar', value: 'x2', inline: true },
        { name: '📦 Docena (1-12, 13-24, 25-36)', value: 'x3', inline: true },
      ],
...
progressQuest(userId, 'pack_opened', 1);
  if (['gold','legend','icon'].includes(type)) {
    ...
    const rarityBadge  = { 'FazeChampions': '🔴 FAZE CHAMPIONS', 'Icon':'⭐ ICON', 'WorldCup':'🏆 WORLD CUP', 'Legendario':'👑 LEGENDARIO', 'Epico':'💜 ÉPICO', 'Raro':'💙 RARO', 'Comun':'🤍 COMÚN' };
    const rarityEmojis = { 'FazeChampions':'🔴', 'Icon':'⭐', 'WorldCup':'🏆', 'Legendario':'✨', 'Epico':'💜', 'Raro':'💙', 'Comun':'⚪' };
...
if (cmd === '!mejorar') {
  ...
  const rarityColors = { 'FazeChampions': 0x8B0000,'Icon': 0xC0C0C0, 'WorldCup': 0xCC2200, 'Legendario': 0xFFD700, 'Epico': 0x9B59B6, 'Raro': 0x5B9BD5, 'Comun': 0x8B7355 };
  const rarityEmoji  = { 'FazeChampions': '🔴', 'Icon': '⭐', 'WorldCup': '🏆', 'Legendario': '👑', 'Epico': '💜', 'Raro': '💙', 'Comun': '⚪' };
...
if (cmd === '!trabajo') {
  ...
  // Base por ELO
  const eloBonus = Math.floor(elo / 20); // 100 ELO → 10 coins base
...
if (cmd === '!logros') {
  ...
  const desbloqueados = LOGROS_DEF.filter(l => u.logros[l.id]?.unlocked).length;
  const porcentaje   = Math.round((desbloqueados / total) * 100);
  const totalCoins   = LOGROS_DEF.filter(l => u.logros[l.id]?.unlocked)
                                  .reduce((s, l) => s + (l.reward?.coins || 0), 0);
 
  // Barra de progreso global
  const barFilled = Math.round((desbloqueados / total) * 15);
  const barEmpty  = 15 - barFilled;
  const globalBar = `\`${'█'.repeat(barFilled)}${'░'.repeat(barEmpty)}\` **${desbloqueados}/${total}** (${porcentaje}%)`;
 
  // Agrupar por categoría
  const categorias = [
    { label: '⚔️ Partidos & Arena', tipos: ['arena_win', 'friendly_play'] },
    { label: '📅 Constancia',       tipos: ['daily_streak'] },
    { label: '📦 Colección',        tipos: ['packs_opened', 'rarity_owned', 'club_full'] },
    { label: '💰 Economía',         tipos: ['coins_total', 'cards_sold'] },
    { label: '📊 ELO',              tipos: ['elo_reached'] },
    { label: '🥊 Duelos',           tipos: ['duels_played', 'duels_won'] },
  ];
 
  // Páginas: una por categoría
  let lPage = 0;
  const totalPages = categorias.length;
 
  function buildLogrosEmbed(page) {
    const cat = categorias[page];
    const catLogros = LOGROS_DEF.filter(l => cat.tipos.includes(l.tipo));
 
    const lines = catLogros.map(l => {
      const unlocked = !!u.logros[l.id]?.unlocked;
      const progress = u.logrosStats?.[l.id] || 0;
      const objNum   = typeof l.objetivo === 'number' ? l.objetivo : 1;
 
      if (unlocked) {
        const fecha = new Date(u.logros[l.id].date).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: '2-digit' });
        return `${l.emoji} ~~**${l.nombre}**~~ ✅ _${fecha}_ · +${(l.reward?.coins || 0).toLocaleString()} 💰`;
      }
 
      // Barra de progreso individual (solo logros acumulativos)
      let progressLine = '';
    ...
    const unlockedCat = catLogros.filter(l => u.logros[l.id]?.unlocked).length;
 
    return {
      embeds: [{
        color: unlockedCat === catLogros.length ? 0x00C851 : 0x2b2d31,
        author: {
          name: `🏆 Logros de ${message.author.username}`,
          icon_url: message.author.displayAvatarURL({ dynamic: true })
        },
        title: `${cat.label}  ·  ${unlockedCat}/${catLogros.length}`,
        description: lines || '_Sin logros en esta categoría._',
        fields: [
          {
          ...
          {
            name: '💰 Monedas ganadas con logros',
        ...
        footer: { text: `Página ${page + 1}/${totalPages}  ·  Navega con los botones  ·  Los premios se cobran automáticamente` },
        timestamp: new Date().toISOString()
      }]
    };
  }
 
  function buildLogrosRow(uid, page) {
    return new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`logros_prev_${uid}`)
        .setLabel('⬅️ Anterior')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(page === 0),
      new ButtonBuilder()
        .setCustomId(`logros_page_${uid}`)
        .setLabel(`${page + 1} / ${totalPages}`)
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(true),
      new ButtonBuilder()
        .setCustomId(`logros_next_${uid}`)
  ...
  const logrosMsg = await message.reply({
    ...buildLogrosEmbed(lPage),
    components: [buildLogrosRow(userId, lPage)]
  });
 
  const lCol = logrosMsg.createMessageComponentCollector({ time: 120000 });
  lCol.on('collect', async interaction => {
    if (interaction.user.id !== userId)
      return interaction.reply({ content: '❌ Este panel no es tuyo.', ephemeral: true });
    if (interaction.customId === `logros_next_${userId}` && lPage < totalPages - 1) lPage++;
    else if (interaction.customId === `logros_prev_${userId}` && lPage > 0) lPage--;
    await interaction.update({ ...buildLogrosEmbed(lPage), components: [buildLogrosRow(userId, lPage)] });
  });
  lCol.on('end', () => logrosMsg.edit({ components: [] }).catch(() => {}));
  return;
}
...
if (cmd === '!coleccion') {
  ...
  const rarezas = ['Icon', 'WorldCup', 'Legendario', 'Epico', 'Raro', 'Comun'];
  const rarityEmoji = {
    'Icon': '⭐', 'WorldCup': '🏆', 'Legendario': '👑',
    'Epico': '💜', 'Raro': '💙', 'Comun': '⚪'
  };
  ...
  // Cartas únicas poseídas más raras
  const topCards = [...myPlayers]
    .sort((a, b) => b.rating - a.rating)
    .filter((p, i, arr) => arr.findIndex(x => x.name === p.name) === i) // deduplicar
    .slice(0, 5)
    .map(p => `${rarityEmoji[p.rarity]} **${p.name}** · ${p.rating} OVR`)
    .join('\n');
 
  const rarityLines = rarezas.map(r => {
    const s = stats[r];
  ...
  return message.reply({
    embeds: [{
      ...
      author: {
        name: `📊 Colección de ${message.author.username}`,
        icon_url: message.author.displayAvatarURL({ dynamic: true })
      },
      ...
      fields: [
        {
          name: '📦 Desglose por rareza',
          value: rarityLines,
          inline: false
        },
        {
          name: '⭐ Tus 5 mejores cartas únicas',
          value: topCards || '_Sin jugadores en el club_',
          inline: false
        },
        {
          name: '🃏 Cartas en club',
          value: `**${myPlayers.length}** (con duplicados)`,
          inline: true
        },
        {
          name: '🔑 Cartas únicas',
          value: `**${ownedAll}** de **${totalAll}**`,
          inline: true
        },
        {
          name: '📈 Completado',
      ...
      footer: { text: '!players para ver el catálogo completo  ·  !market para conseguir cartas que te faltan' },
      timestamp: new Date().toISOString()
    }]
  });
}
...
if (cmd === '!duelo') {
  ...
  duelCol.on('collect', async interaction => {
    ...
    if (interaction.customId === `duelo_accept_${userId}_${target.id}_${bet}`) {
      ...
      // ── Motor de partido ──
      const RARITY_BONUS = {
        "Comun": 0.00, "Raro": 0.05, "Epico": 0.10,
        "Legendario": 0.18, "WorldCup": 0.48, "Icon": 0.56,
      };
...
progressQuest(userId, 'market_visited', 1);
  ...
  .sort((a, b) => a.price - b.price);
    ...
    const rarityColors = { 'FazeChampions': 0x8B0000, 'Icon': 0xC0C0C0, 'WorldCup': 0xCC2200, 'Legendario': 0xFFD700, 'Epico': 0x9B59B6, 'Raro': 0x5B9BD5, 'Comun': 0x8B7355 };
    const rarityEmoji  = { 'FazeChampions': '🔴', "Icon": '⭐ ICON', "WorldCup": '🏆 WORLD CUP', "Legendario": '👑 LEGENDARIO', "Epico": '💜 ÉPICO', "Raro": '💙 RARO', "Comun": '⚪ COMÚN' };
    ...
    const inviteMsg = await message.reply({
      ...
      embeds: [{
        ...
        description: [
          ...
          `📊 ELO del clan: ${clanTier.emoji} **${myClan.elo || 1000}**`,
        ].join('\n'),
        footer: { text: '⏱️ Tienes 120 segundos para responder' },
        timestamp: new Date().toISOString()
      }],
    ...
    return message.reply({
      embeds: [{
        ...
        fields: [
          ...
          { name: '📊 ELO del clan', value: `${getEloTier(targetClan.elo || 1000).emoji} **${targetClan.elo || 1000}**`, inline: true },
...
const lines = ranking.map((r, i) => {
  ...
  const num = i < 3 ? medals[i] : `**${i + 1}.**`;
  return `${num} **${r.clan.name}** — ${tier.emoji} **${r.elo}** ELO · ${r.clan.members.length} miembros · ✅${r.clan.warWins}W ❌${r.clan.warLosses}L`;
}).join('\n');
...
// Buscar rival por ELO de clan más cercano
candidates.sort((a, b) => {
...
if (cmd === '!players') {
  ...
  let filteredPlayers = [...players].filter(p => p.rarity !== 'WorldCup' && p.rarity !== 'Especial');
  ...
  async function buildPlayersCanvas(page) {
    ...
    const rarityColor = { 'FazeChampions': '#CC0000', 'Icon': '#C0C0C0', "WorldCup": '#CC2200', "Legendario": '#FFD700', "Epico": '#9B59B6', "Raro": '#5B9BD5', "Comun": '#A0836A' };
const rarityEmoji = { "FazeChampions": '🔴', "Icon": '⭐', "WorldCup": '🏆', "Legendario": '👑', "Epico": '💜', "Raro": '💙', "Comun": '⚪' };
const posEmoji    = { GK: '🧤', DM: '🛡️', AM: '🎯', ST: '⚽' };
const packForRarity = { "FazeChampions": '🔴 FaZe', "Icon": '⭐ Icon', "WorldCup": '🏆 World Cup', "Legendario": '💎 Legend', "Epico": '🥇 Gold', "Raro": '🥈 Silver', "Comun": '🥉 Bronze' };
...

{
     ...
     value: 'Usa `!duelo @usuario <apuesta>` · Mín **100 💰** · Cooldown 5 min',
     inline: false
   },

{
  ...
  value: 'Usa `!trivia` · Premio hasta **250 💰** · Cooldown 15 min',
  inline: false
},

        {
  name: `🎰 Ruleta — ${rulStr}`,
  value: rulReady ? `Usa \`!rul <cantidad> <color/número>\`` : `Vuelve en **${rulMM}m ${rulSS}s**`,
  inline: false
},
{
  name: `🎟️ Raspadito — ${rasparStr}`,
  value: rasparReady ? `Usa \`!raspar\` · Costo: **200 💰**` : `Vuelve en **${rasparMM}m ${rasparSS}s**`,
  inline: false
},
{
  name: `🎲 Dados — ${dadosStr}`,
...
// ── !torneo forzar <id> ──
    if (sub === 'forzar') {
      const tId = args[2];
      if (!isTournamentAdmin(userId)) return message.reply('❌ Solo admins.');
      const t = tournaments[tId];
...
// ─────────────────────────────────────────
  ...
  if (isAdmin(userId)) {
    ...
    if (cmd === '!givepack') {
      ...
      if (!target || !packs[type]) return message.reply('❌ Uso: `!givepack @usuario silver/bronze/gold/legend/faze/icon [cantidad]`');
      if (!data[target.id]) data[target.id] = { coins: 0, players: [], team: [], packs: { silver: 0, bronze: 0, gold: 0, legend: 0 }, elo: 1000, daily: { lastClaim: 0, streak: 0 } };
